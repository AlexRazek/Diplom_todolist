...
class Command(BaseCommand):
    ...
    def handle_goal_list(self, msg: Message, tg_user: TgUser):
        ...
        self.tg_client.send_message(msg.chat.id, '\n'.join(resp_goals) or '[goals не найдены]')
    ...
    def handle_save_selected_category(self, msg: Message, tg_user: TgUser):
        if msg.text.isdigit():
            ...
            if GoalCategory.objects.filter(
                ...
                return
        self.tg_client.send_message(msg.chat.id, '[выбрана не верная категория]')
    ...
    def handle_save_new_goal(self, msg: Message, tg_user: TgUser):
        ...
        if goal.complete():
            ...
            self.tg_client.send_message(msg.chat.id, text='[Новая цель создана]')
        else:
            self.tg_client.send_message(msg.chat.id, text='[Что-то пошло не так]')
